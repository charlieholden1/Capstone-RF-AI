...
MODEL_FILE = "../data/spectraguard_model.hef"
...
JAM_DETECTION_COUNT = 1       # consecutive detections required before acting
...
jam_counters = {}
pending_hop_freq = None       # tracks requested hop until nodes confirm via FREQ_CHANGED
...
def publish_hop(freq, node_id=None, per_node=True):
    """Publish a hop command. Sends broadcast `HOP:ALL:<freq>` and optionally per-node `HOP:<NODE>:<freq>`.
    Returns True if at least one publish succeeded.
    """
    global last_hop_time, current_network_freq, pending_hop_freq
    try:
...
def on_mqtt_message(client, userdata, msg):
    global current_network_freq, pending_hop_freq
    try:
...
def status_manager():
    ...
    Aggressively pings every 5s for the first 30s, then every 60s as a health check.
    """
    start = time.time()
